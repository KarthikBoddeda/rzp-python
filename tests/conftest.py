...
from rzp import Rzp, AsyncRzp, DefaultAioHttpClient
from rzp._utils import is_dict
...
logging.getLogger("rzp").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Rzp]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Rzp(base_url=base_url, username=username, password=password, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncRzp]:
